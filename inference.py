...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            for newP, prob in self.getPositionDistribution(self.setGhostPosition(gameState, p)).items():
                allPossible[newP] += prob*self.beliefs[p]
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        i = 0
        while i < numParticles / len(legal):
            j = 0
            while j < len(legal):
                self.particles.append(legal[j]) 
                j += 1
            i += 1
