...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for counter in livingGhostPositionDistributions:
            ghostPositions.append(counter.argMax())
            
        for p in ghostPositions:
            distances.append(self.distancer.getDistance(pacmanPosition, p))
            index = distances.index(min(distances))
            
        closestP = ghostPositions[index]

        actionL = []
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            actionL.append(self.distancer.getDistance(successorPosition, closestP))
            
        return legal[actionL.index(min(actionL))]
